run_salmon <- function(transcript_file, bam_files){
    ...
    current_command <- paste("salmon quant -t", transcript_file,
                             "-l U -a", bam_files,
                             "-o", paste0("./salmon_out/", basename(tools::file_path_sans_ext(bam_files)))) # nolint: line_length_linter.

    # Run the current command
    system(current_command)

    # List all the output files from the command that just ran
    all_output <- list.files(here::here("salmon_out", basename(tools::file_path_sans_ext(bam_files))), # nolint: line_length_linter.
                            recursive = TRUE,
                            full.names = TRUE)
